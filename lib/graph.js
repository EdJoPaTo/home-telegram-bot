...
class Graph {
  ...
  addSeries(position) {
    this.dataPromises.push(loadData(position, this.type))
  }

  async create(timeframeInMs) {
    this.dir = await fsPromises.mkdtemp('./tmp/')
    const data = await Promise.all(this.dataPromises)
    const svgString = create(timeframeInMs, this.unit, ...data)
...
async function loadData(position, type) {
  const content = await fsPromises.readFile(`data/${position}/${type}.log`, 'utf8')
  const data = await d3.csvParseRows(content, (d, i) => {
...
function create(timeframe, unit, ...series) {
  ...
  for (let i = 0; i < series.length; i++) {
    const lastValue = series[i].data.slice(-1)[0].value
